...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.23.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function render_materials() {
		...
		if ( ! $product_image ) { $product_image = wc_placeholder_img_src( 'large' ); }
		$price = $product->get_price_html();
		$html = '<div class="cns-material-selector" data-product-name="' . esc_attr( $product->get_name() ) . '" data-product-image="' . esc_url( $product_image ) . '" data-product-price="' . esc_attr( wp_strip_all_tags( $price ) ) . '">';
		$html .= '<style>.cns-material-selector{padding:8px 0}.cns-material-selector .cns-mat-group{margin:0 0 30px}.cns-material-selector .cns-mat-group-title{font-size:16px;font-weight:600;margin-bottom:12px}.cns-material-selector .cns-mat-help{font-size:12px;color:#777;margin:-7px 0 13px}.cns-material-selector .cns-mat-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:12px}.cns-q-material{display:block;text-align:left;width:100%;padding:0;background:#fff;border:1px solid #e4e4e4;cursor:pointer;transition:.18s}.cns-q-material.is-selected{border:2px solid #161616;box-shadow:0 0 0 2px #fff inset}.cns-q-material .cns-mat-thumb{display:block;aspect-ratio:1;overflow:hidden;background:#eee}.cns-q-material img,.cns-q-material .cns-mat-swatch{width:100%;height:100%;display:block;object-fit:cover}.cns-q-material .cns-mat-name{font-size:13px;line-height:1.35;padding:9px 9px 2px;font-weight:600}.cns-q-material .cns-mat-code{font-size:11px;color:#777;padding:0 9px 9px}.cns-quote-action{display:none;margin:8px 0 0;padding:13px 18px;background:#1b1b1b;border:0;color:#fff;cursor:pointer;font-weight:600}.cns-quote-action.is-visible{display:inline-block}</style>';
		$html .= '<p class="cns-mat-help" style="margin:0 0 20px;">Chọn tối đa một vật liệu cho mỗi danh mục.</p>';
		foreach ( $groups as $type => $items ) {
			$html .= '<div class="cns-mat-group" data-material-type="' . esc_attr( $type ) . '"><div class="cns-mat-group-title">' . esc_html( $type ) . '</div><p class="cns-mat-help">Chọn 1 vật liệu</p><div class="cns-mat-grid">';
			foreach ( $items as $m ) {
				$thumb = ! empty($m['thumb']) ? $m['thumb'] : ''; $name = isset($m['name']) ? $m['name'] : 'Vật liệu'; $code = ! empty($m['code']) ? $m['code'] : '';
				$visual = $thumb ? '<img src="' . esc_url($thumb) . '" alt="' . esc_attr($name) . '" loading="lazy" />' : '<span class="cns-mat-swatch" style="background:' . esc_attr($this->color_hex(isset($m['color'])?$m['color']:'')) . ';"></span>';
				$html .= '<button type="button" class="cns-q-material" data-type="' . esc_attr($type) . '" data-name="' . esc_attr($name) . '" data-code="' . esc_attr($code) . '" data-image="' . esc_url($thumb) . '"><span class="cns-mat-thumb">' . $visual . '</span><span class="cns-mat-name">' . esc_html($name) . '</span>' . ($code?'<span class="cns-mat-code">'.esc_html($code).'</span>':'') . '</button>';
			}
			$html .= '</div></div>';
		}
		$html .= '<button type="button" class="cns-quote-action">Xuất PDF báo giá</button>';
		$html .= '<script>(function(){var root=document.currentScript.parentElement,selected={};root.addEventListener("click",function(e){var card=e.target.closest(".cns-q-material");if(card){var type=card.dataset.type;root.querySelectorAll(".cns-q-material[data-type=\\\""+CSS.escape(type)+"\\\"]").forEach(function(x){x.classList.remove("is-selected")});card.classList.add("is-selected");selected[type]={type:type,name:card.dataset.name,code:card.dataset.code,image:card.dataset.image};root.querySelector(".cns-quote-action").classList.toggle("is-visible",Object.keys(selected).length>0);return}if(e.target.closest(".cns-quote-action")){var rows=Object.keys(selected).map(function(k){var m=selected[k],img=m.image?"<img src=\\\""+m.image+"\\\" style=\\\"width:64px;height:64px;object-fit:cover\\\">":"";return "<tr><td>"+img+"</td><td><b>"+m.type+"</b><br>"+m.name+(m.code?"<br><small>"+m.code+"</small>":"")+"</td></tr>"}).join("");var w=window.open("","_blank");w.document.write("<!doctype html><html><head><title>Báo giá - "+root.dataset.productName+"</title><style>body{font-family:Arial,sans-serif;color:#191919;margin:42px}h1{font-size:26px;margin-bottom:7px}.hero{width:300px;max-height:300px;object-fit:cover;margin:20px 0}table{border-collapse:collapse;width:100%;margin-top:18px}td{border-top:1px solid #ddd;padding:12px;vertical-align:middle}small{color:#666}.note{margin-top:28px;color:#666;font-size:13px}@media print{body{margin:20px}}</style></head><body><h1>BÁO GIÁ</h1><p>Ngày xuất: "+new Date().toLocaleDateString("vi-VN")+"</p><h2>"+root.dataset.productName+"</h2><img class=\\\"hero\\\" src=\\\""+root.dataset.productImage+"\\\"><p><b>Giá tham khảo:</b> "+(root.dataset.productPrice||"Liên hệ để nhận báo giá")+"</p><h3>Vật liệu đã chọn</h3><table>"+rows+"</table><p class=\\\"note\\\">Báo giá được tạo từ lựa chọn vật liệu trên COLECTIA. Vui lòng liên hệ để xác nhận giá và thời gian giao hàng.</p><script>window.onload=function(){window.print()}<\\/script></body></html>");w.document.close()}})})();</script></div>';
		return $html;
	}
