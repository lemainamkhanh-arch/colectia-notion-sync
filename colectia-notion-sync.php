...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.28.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function project_cart_styles() { if ( ! function_exists('is_cart') || ! is_cart() ) { return; } echo '<style>.woocommerce-cart-form .product-quantity,.cart-collaterals .cart-subtotal,.cart-collaterals .order-total{display:none!important}.woocommerce-cart-form{border-top:1px solid #e4e0da}.woocommerce-cart-form__cart-item td{padding:24px 12px!important;border-bottom:1px solid #e4e0da}.woocommerce-cart-form__cart-item .product-name a{font-size:16px;font-weight:500;color:#1d1d1b}.woocommerce-cart-form__cart-item .variation{margin-top:9px;font-size:12px;color:#777}.cart-collaterals{background:#f5f3ef;padding:28px}.cart-collaterals h2{font-size:13px;letter-spacing:.12em;text-transform:uppercase}.cns-project-quote{background:#1b1b19!important;border:1px solid #1b1b19!important;color:#fff!important;padding:15px 20px!important;font-size:12px!important;letter-spacing:.08em;text-transform:uppercase}.cns-project-quote:hover{background:transparent!important;color:#1b1b19!important}</style>'; }
	...
	public function project_quote_button() {
		...
		if(!$items)return; echo '<a href="#" class="checkout-button button alt cns-project-quote" data-project="'.esc_attr(wp_json_encode($items)).'">Xuất báo giá dự án</a>';
		echo '<script>(function(){var b=document.querySelector(".cns-project-quote");if(!b)return;b.addEventListener("click",function(e){e.preventDefault();var items=JSON.parse(b.dataset.project||"[]"),esc=function(v){var d=document.createElement("div");d.textContent=v||"";return d.innerHTML},content=items.map(function(item,i){var mats=(item.materials||[]).map(function(m){return `<li>${m.image?`<img src="${esc(m.image)}">`:""}<span><b>${esc(m.type||"Vật liệu")}</b><br>${esc(m.name)}${m.code?` · ${esc(m.code)}`:""}</span></li>`}).join("");return `<section class="item"><div class="product"><span class="num">${String(i+1).padStart(2,"0")}</span><img src="${esc(item.image)}"><h2>${esc(item.name)}</h2></div>${mats?`<div class="materials"><h3>Vật liệu đã chọn</h3><ul>${mats}</ul></div>`:""}</section>`}).join(""),w=window.open("","_blank");if(!w)return;w.document.write(`<!doctype html><html><head><title>Báo giá dự án COLECTIA</title><style>@page{size:A4;margin:0}*{box-sizing:border-box}body{margin:0;background:#f4f2ee;font-family:"Helvetica Neue",Arial,Helvetica,sans-serif;color:#20201e}.sheet{width:210mm;min-height:297mm;background:#fff;margin:auto;padding:21mm 20mm}.head{position:relative;text-align:center;border-bottom:1px solid #d9d5cf;padding-bottom:14mm}.logo{position:absolute;left:0;top:0;width:142px;height:32px;object-fit:contain;object-position:left}.eyebrow{font-size:10px;letter-spacing:.16em;color:#77736c;text-transform:uppercase;margin:13px 0 6px}.title{font-size:30px;letter-spacing:.08em;font-weight:500;margin:0}.date{position:absolute;right:0;top:0;font-size:11px;color:#77736c;line-height:1.6;text-align:right}.item{padding:13mm 0;border-bottom:1px solid #e4e0da}.product{display:flex;align-items:center;gap:13px}.num{font-size:10px;color:#77736c}.product>img{width:72px;height:72px;object-fit:cover}.product h2{font-size:18px;font-weight:500;margin:0}.materials{margin:10px 0 0 99px}.materials h3{font-size:9px;letter-spacing:.13em;text-transform:uppercase;color:#77736c;margin:0 0 7px}.materials ul{list-style:none;margin:0;padding:0;display:flex;flex-wrap:wrap;gap:9px}.materials li{display:flex;align-items:center;gap:7px;font-size:10px;line-height:1.35}.materials li img{width:30px;height:30px;object-fit:cover}.materials b{font-size:8px;letter-spacing:.1em;text-transform:uppercase;color:#77736c}.foot{margin-top:18mm;border-top:1px solid #d9d5cf;padding-top:7mm;display:flex;justify-content:space-between;font-size:9px;line-height:1.6;color:#77736c}@media print{body{background:#fff}.sheet{margin:0}}</style></head><body><main class="sheet"><header class="head"><img class="logo" src="https://colectia.vn/wp-content/uploads/2025/10/Logo.svg"><p class="eyebrow">Project selection</p><h1 class="title">BÁO GIÁ</h1><p class="date">Ngày xuất<br><b>${new Date().toLocaleDateString("vi-VN")}</b></p></header>${content}<footer class="foot"><span>COLECTIA VIETNAM<br>colectia.vn</span><span>Báo giá được tạo từ danh sách Dự án.<br>Vui lòng liên hệ để xác nhận báo giá.</span></footer></main></body></html>`);w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>';
	}
	...
	public function render_materials() {
		...
		$html .= '<button type="button" class="cns-quote-action"><span>Xuất PDF báo giá</span><span>→</span></button>';
