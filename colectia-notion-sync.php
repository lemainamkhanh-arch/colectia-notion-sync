...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.39.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function render_moodboard_account_page() { $materials=$this->get_project_moodboard_materials();echo '<div class="cns-account-moodboard"><div class="cns-account-section-head"><span>Material palette</span><h2>Moodboard của bạn</h2><p>Lưu trữ các vật liệu đã chọn cho dự án và không gian của bạn.</p></div>';if(!$materials){echo '<div class="cns-account-empty"><p>Chưa có vật liệu nào trong Moodboard.</p><a class="button" href="'.esc_url(get_post_type_archive_link(self::CPT_MAT)).'">Khám phá vật liệu</a></div>';}else{echo '<div class="cns-account-mood-grid">';foreach($materials as $m){echo '<article class="cns-account-mood-card">'.(!empty($m['image'])?'<img src="'.esc_url($m['image']).'" alt="'.esc_attr($m['name']).'">':'<span class="cns-mood-placeholder"></span>').'<div><small>'.esc_html($m['type']).'</small><h3>'.esc_html($m['name']).'</h3>'.(!empty($m['code'])?'<em>'.esc_html($m['code']).'</em>':'').'</div></article>';}echo '</div>';}echo '</div><style>.cns-account-section-head{padding-bottom:24px;border-bottom:1px solid #dedbd5;margin-bottom:24px}.cns-account-section-head>span{font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:#777}.cns-account-section-head h2{font-size:28px;font-weight:500;margin:6px 0}.cns-account-section-head p{font-size:13px;color:#777;margin:0}.cns-account-mood-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px}.cns-account-mood-card{border:1px solid #e3dfd8;background:#fff}.cns-account-mood-card>img,.cns-account-mood-card>.cns-mood-placeholder{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#eeeae4}.cns-account-mood-card>div{padding:14px}.cns-account-mood-card small{font-size:8px;letter-spacing:.12em;text-transform:uppercase;color:#888}.cns-account-mood-card h3{font-size:14px;font-weight:500;margin:5px 0}.cns-account-mood-card em{font-size:11px;color:#777;font-style:normal}.cns-account-empty{padding:35px;border:1px solid #e3dfd8;background:#faf9f7;text-align:center}@media(max-width:767px){.cns-account-mood-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}</style>'; }
