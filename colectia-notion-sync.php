...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.41.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
public function render_moodboard_account_page() {
    ...
    if(!$materials){
        ...
        echo '<section class="cns-mood-preview-wrap"><div class="cns-mood-list-head"><span>PREVIEW</span><span>'.esc_html(count($materials)).' VẬT LIỆU</span></div><div class="cns-mood-preview">';
        foreach($materials as $m){
            $type=isset($m['type'])?(string)$m['type']:'';
            $key=function_exists('mb_strtolower')?mb_strtolower($type,'UTF-8'):strtolower($type);
            $kind=(strpos($key,'đá')!==false||strpos($key,'stone')!==false)?' is-stone':((strpos($key,'da')!==false||strpos($key,'leather')!==false)?' is-leather':'');
            echo '<figure class="cns-mood-preview-card'.$kind.'">'.(!empty($m['image'])?'<img src="'.esc_url($m['image']).'" alt="'.esc_attr($m['name']).'">':'<span></span>').'</figure>';
        }
...
public function moodboard_export_script() {
    ...
    ?>
<script>(function(){var b=document.querySelector('.cns-export-moodboard');if(!b)return;b.addEventListener('click',function(){var board=document.querySelector('.cns-mood-preview');if(!board)return;var w=window.open('','_blank');if(!w)return;var h='<!doctype html><html><head><title>Moodboard COLECTIA</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;color:#1b1b19}.head{display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:10mm;border-bottom:1px solid #d8d4cd}.eyebrow{font-size:8px;letter-spacing:.18em;color:#777}.title{font-size:25px;font-weight:500;margin:7px 0 0}.date{font-size:9px;color:#777}.cns-mood-preview{display:grid;grid-template-columns:repeat(4,1fr);gap:13px;padding:18px;background:#eeece7;margin-top:10mm}.cns-mood-preview-card{margin:0;background:#fff;border:1px solid rgba(40,38,34,.12)}.cns-mood-preview-card img,.cns-mood-preview-card span{display:block;width:100%;aspect-ratio:1;object-fit:cover}.cns-mood-preview-card.is-stone,.cns-mood-preview-card.is-leather{box-shadow:0 11px 20px rgba(44,39,32,.24);transform:translateY(-5px) rotate(-1.1deg)}.cns-mood-preview-card.is-leather{transform:translateY(5px) rotate(1.1deg)}.foot{margin-top:10mm;padding-top:5mm;border-top:1px solid #d8d4cd;font-size:8px;color:#777}</style></head><body><header class="head"><div><span class="eyebrow">COLECTIA · MATERIAL PALETTE</span><h1 class="title">MOODBOARD</h1></div><span class="date">'+new Date().toLocaleDateString('vi-VN')+'</span></header>'+board.outerHTML+'<footer class="foot">COLECTIA VIETNAM · colectia.vn</footer></body></html>';w.document.write(h);w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>
<?php
}
