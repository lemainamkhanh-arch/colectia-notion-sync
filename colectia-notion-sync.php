...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.41.2'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
public function moodboard_export_script() {
    ...
    ?>
<script>(function(){var b=document.querySelector('.cns-export-moodboard');if(!b)return;function esc(v){var d=document.createElement('div');d.textContent=v||'';return d.innerHTML}b.addEventListener('click',function(){var items=[];try{var raw=atob(b.dataset.materials||'');var bytes=Uint8Array.from(raw,function(c){return c.charCodeAt(0)});items=JSON.parse(new TextDecoder().decode(bytes))}catch(e){return}if(!items.length)return;var slots=[[4,5,-2],[51,4,1.5],[20,36,-1],[62,35,2],[37,18,.7],[3,62,1.2],[69,64,-1.6],[40,65,2.2],[13,20,1],[55,20,-2],[25,66,.5],[72,12,1.3]];var cards=items.map(function(m,i){var p=slots[i%slots.length],size=m.kind==='soft'?32:(m.kind==='stone'?27:23),shadow=m.kind==='soft'?'0 7px 14px rgba(35,31,26,.15)':(m.kind==='stone'?'0 17px 28px rgba(35,31,26,.28),0 4px 7px rgba(35,31,26,.16)':'none');return '<figure class="sample" style="left:'+p[0]+'%;top:'+p[1]+'%;width:'+size+'%;transform:rotate('+p[2]+'deg);box-shadow:'+shadow+'">'+(m.image?'<img src="'+esc(m.image)+'">':'<span></span>')+'</figure>'}).join('');var listing=items.map(function(m){return '<article><div class="thumb">'+(m.image?'<img src="'+esc(m.image)+'">':'')+'</div><small>'+esc(m.type)+'</small><b>'+esc(m.name)+'</b>'+(m.code?'<em>'+esc(m.code)+'</em>':'')+'</article>'}).join('');var w=window.open('','_blank');if(!w)return;var h='<!doctype html><html><head><title>Moodboard COLECTIA</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{margin:0;background:#fff;font-family:Arial,Helvetica,sans-serif;color:#1b1b19}.head{display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:8mm;border-bottom:1px solid #d8d4cd}.eyebrow{font-size:8px;letter-spacing:.18em;color:#777}.title{font-size:25px;font-weight:500;margin:7px 0 0}.date{font-size:9px;color:#777}.visual{position:relative;height:158mm;margin-top:8mm;background:#fff;overflow:hidden}.sample{position:absolute;margin:0;background:#fff}.sample img,.sample span{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#f4f2ee}.list-head{display:flex;justify-content:space-between;padding:0 0 3mm;border-bottom:1px solid #d8d4cd;font-size:7px;letter-spacing:.14em;color:#777}.listing{display:grid;grid-template-columns:repeat(4,1fr);gap:4mm;margin-top:4mm}.listing article{break-inside:avoid}.thumb{width:100%;aspect-ratio:1;background:#f3f1ed;margin-bottom:2mm}.thumb img{width:100%;height:100%;object-fit:cover}.listing small,.listing b,.listing em{display:block}.listing small{font-size:6px;letter-spacing:.1em;text-transform:uppercase;color:#888}.listing b{font-size:8px;font-weight:500;margin-top:1mm}.listing em{font-size:7px;color:#777;font-style:normal;margin-top:.5mm}.foot{margin-top:8mm;padding-top:4mm;border-top:1px solid #d8d4cd;font-size:7px;color:#777}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><header class="head"><div><span class="eyebrow">COLECTIA · MATERIAL PALETTE</span><h1 class="title">MOODBOARD</h1></div><span class="date">'+new Date().toLocaleDateString('vi-VN')+'</span></header><section class="visual">'+cards+'</section><div class="list-head"><span>DANH SÁCH VẬT LIỆU</span><span>'+items.length+' VẬT LIỆU</span></div><section class="listing">'+listing+'</section><footer class="foot">COLECTIA VIETNAM · colectia.vn</footer></body></html>';w.document.write(h);w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>
<?php
}
