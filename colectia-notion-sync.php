...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.40.1'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
\t\t$materials=$this->get_project_moodboard_materials();$archive=get_post_type_archive_link(self::CPT_MAT);$count=count($materials);
...
\t\tif(!$materials){echo '<div class="cns-account-empty"><span>—</span><h3>Moodboard đang trống</h3><p>Lưu các vật liệu yêu thích để tạo bảng cảm hứng riêng.</p><a class="cns-btn cns-btn-dark" href="'.esc_url($archive).'">KHÁM PHÁ VẬT LIỆU</a></div>';}else{
\t\t\techo '<section class="cns-mood-preview-wrap"><div class="cns-mood-list-head"><span>PREVIEW</span><span>'.esc_html($count).' VẬT LIỆU</span></div><div class="cns-mood-preview">';
\t\t\tforeach($materials as $m){$type=isset($m['type'])?(string)$m['type']:'';$type_key=function_exists('mb_strtolower')?mb_strtolower($type,'UTF-8'):strtolower($type);$kind='';if(strpos($type_key,'đá')!==false||strpos($type_key,'stone')!==false)$kind=' is-stone';elseif(strpos($type_key,'da')!==false||strpos($type_key,'leather')!==false)$kind=' is-leather';echo '<figure class="cns-mood-preview-card'.$kind.'">'.(!empty($m['image'])?'<img src="'.esc_url($m['image']).'" alt="'.esc_attr($m['name']).'">':'<span class="cns-mood-placeholder"></span>').'</figure>';}
\t\t\techo '</div></section><section class="cns-mood-listing"><div class="cns-mood-list-head"><span>DANH SÁCH VẬT LIỆU</span><span>MOODBOARD</span></div><div class="cns-account-mood-grid">';
\t\t\tforeach($materials as $m){echo '<article class="cns-account-mood-card">'.(!empty($m['image'])?'<img src="'.esc_url($m['image']).'" alt="'.esc_attr($m['name']).'">':'<span class="cns-mood-placeholder"></span>').'<div><small>'.esc_html(isset($m['type'])?$m['type']:'').'</small><h3>'.esc_html($m['name']).'</h3>'.(!empty($m['code'])?'<em>'.esc_html($m['code']).'</em>':'').'</div></article>';}
\t\t\techo '</div></section><footer class="cns-mood-actions"><div><span class="cns-kicker">Next step</span><h3>Lưu lại bảng cảm hứng<br>cho dự án của bạn.</h3></div><div class="cns-project-action-links"><a href="#" class="cns-btn cns-btn-dark cns-export-moodboard">XUẤT MOODBOARD</a><button type="button" class="cns-btn cns-btn-light cns-request-sample" title="Tính năng yêu cầu sample sẽ sớm được kết nối">YÊU CẦU SAMPLE</button></div></footer>';
\t\t}echo '</div><style>.cns-account-moodboard{max-width:980px;margin:0 auto;color:#1b1b19}.cns-account-section-head{padding:5px 0 27px;border-bottom:1px solid #dedbd5;margin-bottom:28px}.cns-account-section-head>span,.cns-kicker{display:block;font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:#77736c}.cns-account-section-head h2{font-size:32px;line-height:1.1;font-weight:500;letter-spacing:-.025em;margin:8px 0}.cns-account-section-head p{font-size:13px;color:#777;margin:0}.cns-mood-preview-wrap{padding:20px;background:#f0eee9}.cns-mood-list-head{display:flex;justify-content:space-between;align-items:center;padding:0 0 11px;font-size:9px;letter-spacing:.14em;color:#77736c;border-bottom:1px solid #dedbd5}.cns-mood-preview{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;padding-top:14px}.cns-mood-preview-card{display:block;margin:0;background:#e5e1da;aspect-ratio:1;overflow:hidden}.cns-mood-preview-card img,.cns-mood-preview-card .cns-mood-placeholder{display:block;width:100%;height:100%;object-fit:cover;background:#e4e0d8}.cns-mood-listing{margin-top:34px}.cns-account-mood-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;padding-top:14px}.cns-account-mood-card{background:#fff;border:1px solid #e3dfd8}.cns-account-mood-card>img,.cns-account-mood-card>.cns-mood-placeholder{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#eeeae4}.cns-account-mood-card>div{padding:10px}.cns-account-mood-card small{font-size:8px;letter-spacing:.12em;text-transform:uppercase;color:#888}.cns-account-mood-card h3{font-size:12px;line-height:1.25;font-weight:500;margin:4px 0}.cns-account-mood-card em{font-size:10px;color:#777;font-style:normal}.cns-mood-actions{background:#efede8;margin-top:36px;padding:30px;display:flex;justify-content:space-between;align-items:flex-end;gap:25px}.cns-mood-actions h3{font-size:23px;line-height:1.18;font-weight:500;margin:7px 0 0}.cns-mood-actions .cns-project-action-links{display:flex;flex-wrap:wrap;gap:9px}.cns-mood-actions .cns-btn{display:inline-flex;min-height:42px;padding:0 16px;align-items:center;justify-content:center;text-decoration:none!important;font:600 10px/1 inherit;letter-spacing:.1em;border:1px solid #1b1b19}.cns-mood-actions .cns-btn-dark{background:#1b1b19;color:#fff}.cns-mood-actions .cns-btn-dark:hover{background:#fff;color:#1b1b19}.cns-mood-actions .cns-btn-light{background:transparent;color:#1b1b19}.cns-mood-actions .cns-btn-light:hover{background:#fff}.cns-request-sample{appearance:none;cursor:pointer}.cns-account-empty{text-align:center;padding:76px 20px;background:#f7f6f3}.cns-account-empty>span{font-size:32px;color:#b3aea6}.cns-account-empty h3{font-size:24px;font-weight:500;margin:10px 0}.cns-account-empty p{max-width:380px;margin:0 auto 22px;color:#77736c;font-size:13px;line-height:1.6}@media(max-width:700px){.cns-mood-preview-wrap{padding:16px}.cns-mood-preview,.cns-account-mood-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.cns-mood-actions{display:block;padding:24px}.cns-mood-actions .cns-project-action-links{margin-top:20px}.cns-mood-actions .cns-btn{width:100%}}@media(max-width:430px){.cns-mood-preview,.cns-account-mood-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.cns-account-section-head h2{font-size:28px}}</style>'; ?>
<script>(function(){var button=document.querySelector('.cns-export-moodboard');if(!button)return;button.addEventListener('click',function(e){e.preventDefault();var board=document.querySelector('.cns-mood-preview');if(!board)return;var w=window.open('','_blank');if(!w)return;w.document.write('<!doctype html><html><head><title>Moodboard COLECTIA</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#1b1b19;margin:0}.head{display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:10mm;border-bottom:1px solid #d8d4cd}.eyebrow{font-size:8px;letter-spacing:.18em;text-transform:uppercase;color:#777}.title{font-size:25px;font-weight:500;margin:7px 0 0}.date{font-size:9px;color:#777}.cns-mood-preview{display:grid;grid-template-columns:repeat(4,1fr);gap:13px;padding:18px;background:#eeece7;margin-top:10mm;align-items:start}.cns-mood-preview-card{margin:0;background:#fff;border:1px solid rgba(40,38,34,.12)}.cns-mood-preview-card img,.cns-mood-preview-card .cns-mood-placeholder{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#e4e0d8}.cns-mood-preview-card.is-stone,.cns-mood-preview-card.is-leather{box-shadow:0 11px 20px rgba(44,39,32,.24),0 2px 4px rgba(44,39,32,.1);transform:translateY(-5px) rotate(-1.1deg)}.cns-mood-preview-card.is-leather{transform:translateY(5px) rotate(1.1deg);box-shadow:0 12px 21px rgba(44,39,32,.25),0 2px 4px rgba(44,39,32,.1)}.foot{margin-top:10mm;padding-top:5mm;border-top:1px solid #d8d4cd;font-size:8px;color:#777}@media print{body{print-color-adjust:exact;-webkit-print-color-adjust:exact}}</style></head><body><header class="head"><div><span class="eyebrow">COLECTIA · Material palette</span><h1 class="title">MOODBOARD</h1></div><span class="date">Ngày xuất: '+new Date().toLocaleDateString('vi-VN')+'</span></header>'+board.outerHTML+'<footer class="foot">COLECTIA VIETNAM · colectia.vn</footer></body></html>');w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>
<?php }
