...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.24.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function render_materials() {
		...
		$html .= '<script>(function(){var root=document.currentScript.parentElement,selected={};function esc(v){var d=document.createElement("div");d.textContent=v||"";return d.innerHTML}root.addEventListener("click",function(e){var card=e.target.closest(".cns-q-material");if(card&&root.contains(card)){var type=card.dataset.type;root.querySelectorAll(".cns-q-material").forEach(function(item){if(item.dataset.type===type){item.classList.remove("is-selected")}});card.classList.add("is-selected");selected[type]={type:type,name:card.dataset.name,code:card.dataset.code,image:card.dataset.image};root.querySelector(".cns-quote-action").classList.add("is-visible");return}var action=e.target.closest(".cns-quote-action");if(action&&root.contains(action)){var rows=Object.keys(selected).map(function(key){var m=selected[key],image=m.image?`<img src="${esc(m.image)}" style="width:60px;height:60px;object-fit:cover;border-radius:3px">`:"";return `<tr><td style="width:76px">${image}</td><td><strong>${esc(m.type)}</strong><br>${esc(m.name)}${m.code?`<br><small>${esc(m.code)}</small>`:""}</td></tr>`}).join("");if(!rows)return;var w=window.open("","_blank");if(!w)return;var doc=`<!doctype html><html><head><title>Báo giá - ${esc(root.dataset.productName)}</title><style>body{font-family:Arial,sans-serif;color:#1b1b1b;margin:42px;max-width:760px}h1{font-size:28px;letter-spacing:.04em;margin:0 0 8px}.hero{width:280px;max-height:280px;object-fit:cover;margin:20px 0}h2{font-size:22px;margin:0 0 8px}h3{margin-top:30px}table{border-collapse:collapse;width:100%}td{border-top:1px solid #ddd;padding:12px 0;vertical-align:middle}small,.note{color:#666}.note{font-size:12px;margin-top:30px}@media print{body{margin:20px}}</style></head><body><h1>BÁO GIÁ</h1><p>Ngày xuất: ${new Date().toLocaleDateString("vi-VN")}</p><h2>${esc(root.dataset.productName)}</h2><img class="hero" src="${esc(root.dataset.productImage)}"><p><strong>Giá tham khảo:</strong> ${esc(root.dataset.productPrice||"Liên hệ để nhận báo giá")}</p><h3>Vật liệu đã chọn</h3><table>${rows}</table><p class="note">Báo giá được tạo từ lựa chọn vật liệu trên COLECTIA. Vui lòng liên hệ để xác nhận giá và thời gian giao hàng.</p></body></html>`;w.document.open();w.document.write(doc);w.document.close();w.onload=function(){w.focus();w.print()}}})})();</script></div>';
		return $html;
	}
