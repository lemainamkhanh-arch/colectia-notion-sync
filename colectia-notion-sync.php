...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.33.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	private function catalog_mode_hooks() {
		...
		add_filter( 'woocommerce_account_menu_items', array( $this, 'add_project_account_menu' ), 30 );
		...
		add_action( 'woocommerce_after_cart_table', array( $this, 'render_project_moodboard' ), 25 );
	...
	public function replace_download_with_project_button() { if(!function_exists('is_product')||!is_product())return; ?><style>.single-product form.cart .quantity{display:none!important}.cns-download-project-form{margin:0!important;width:100%}.cns-download-project-form .single_add_to_cart_button{width:100%;font-family:inherit!important}</style><script>(function(){var form=document.querySelector('form.cart');if(!form)return;var target=[].slice.call(document.querySelectorAll('a,button')).find(function(el){var t=(el.textContent||'').trim().toLowerCase();return t==='tải xuống'||t==='download'});if(!target)return;var btn=form.querySelector('.single_add_to_cart_button');if(!btn)return;btn.textContent='THÊM VÀO DỰ ÁN';btn.className=target.className+' single_add_to_cart_button button alt';form.classList.add('cns-download-project-form');target.replaceWith(form)})();</script><?php }
	public function register_project_account_endpoint() { add_rewrite_endpoint('du-an',EP_ROOT|EP_PAGES); }
	public function add_project_account_menu( $items ) { $logout=isset($items['customer-logout'])?$items['customer-logout']:null;unset($items['customer-logout']);$items['du-an']='Dự án';if(null!==$logout)$items['customer-logout']=$logout;return $items; }
	public function render_project_account_page() { echo '<div class="cns-account-project"><h2>Dự án của bạn</h2>';if(function_exists('WC')&&WC()->cart&&!WC()->cart->is_empty()){echo do_shortcode('[woocommerce_cart]');}else{echo '<p>Chưa có sản phẩm nào trong dự án.</p><p><a class="button" href="'.esc_url(wc_get_page_permalink('shop')).'">Khám phá sản phẩm</a></p>';}echo '</div>'; }
	...
	private function get_project_moodboard_materials() {
		...
		$ids=(function_exists('WC')&&WC()->session)?(array)WC()->session->get('cns_moodboard_ids',array()):array();foreach(array_unique(array_map('absint',$ids)) as $id){if(self::CPT_MAT!==get_post_type($id))continue;$types=wp_get_object_terms($id,self::TAX_MAT_TYPE,array('fields'=>'names'));$all[]=array('name'=>get_the_title($id),'type'=>(!is_wp_error($types)&&$types)?$types[0]:'','code'=>get_post_meta($id,'_cns_mat_code',true),'image'=>get_the_post_thumbnail_url($id,'medium'));}
	...
	public function append_material_moodboard_button( $content ) { if(!is_singular(self::CPT_MAT)||!in_the_loop()||!is_main_query())return $content;$id=get_the_ID();$ids=(function_exists('WC')&&WC()->session)?(array)WC()->session->get('cns_moodboard_ids',array()):array();$added=in_array($id,array_map('absint',$ids),true);return $content.'<div class="cns-material-moodboard-action"><button type="button" class="cns-add-moodboard'.($added?' is-added':'').'" data-material="'.esc_attr($id).'">'.($added?'ĐÃ CÓ TRONG MOODBOARD':'THÊM VÀO MOODBOARD').'</button></div>'; }
	public function material_moodboard_script() { if(!is_singular(self::CPT_MAT))return;$nonce=wp_create_nonce('cns_moodboard');?><style>.cns-material-moodboard-action{margin:28px 0}.cns-add-moodboard{min-height:44px;padding:0 20px;border:1px solid #1b1b19;background:#1b1b19;color:#fff;font:600 12px/42px inherit;letter-spacing:.09em;cursor:pointer}.cns-add-moodboard:hover,.cns-add-moodboard.is-added{background:#fff;color:#1b1b19}</style><script>(function(){var b=document.querySelector('.cns-add-moodboard');if(!b)return;b.addEventListener('click',function(){if(b.classList.contains('is-added'))return;var d=new URLSearchParams({action:'cns_add_moodboard',nonce:'<?php echo esc_js($nonce); ?>',material_id:b.dataset.material});fetch('<?php echo esc_url(admin_url('admin-ajax.php')); ?>',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:d.toString()}).then(function(r){return r.json()}).then(function(r){if(r.success){b.classList.add('is-added');b.textContent='ĐÃ CÓ TRONG MOODBOARD'}})})})();</script><?php }
	public function ajax_add_moodboard() { check_ajax_referer('cns_moodboard','nonce');$id=isset($_POST['material_id'])?absint($_POST['material_id']):0;if(!$id||self::CPT_MAT!==get_post_type($id))wp_send_json_error();if(!function_exists('WC')||!WC()->session)wp_send_json_error();$ids=(array)WC()->session->get('cns_moodboard_ids',array());$ids[]=$id;WC()->session->set('cns_moodboard_ids',array_values(array_unique(array_map('absint',$ids))));wp_send_json_success(); }
