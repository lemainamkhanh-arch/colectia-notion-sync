...
';
	...
	public function project_quote_button() {
		...
		echo '<script>(function(){var b=document.querySelector(".cns-project-quote");if(!b)return;b.addEventListener("click",function(e){e.preventDefault();var note=document.querySelector("#cns_project_note"),items=JSON.parse(b.dataset.project||"[]"),projectNote=note?note.value:b.dataset.note||"",uname=b.dataset.uname,uemail=b.dataset.uemail,esc=function(v){var d=document.createElement("div");d.textContent=v||"";return d.innerHTML},allMats={};var content=items.map(function(item,i){var mats=(item.materials||[]).map(function(m){var k=(m.name||"")+"-"+(m.code||"");allMats[k]=m;return `<li>${m.image?`<img src="${esc(m.image)}">`:""}<div><b>${esc(m.type||"Vật liệu")}</b><br>${esc(m.name)}${m.code?` <i>${esc(m.code)}</i>`:""}</div></li>`}).join("");return `<article class="item"><div class="product"><span class="num">${String(i+1).padStart(2,"0")}</span><img src="${esc(item.image)}"><div class="prod-meta"><h2>${esc(item.name)}</h2><span class="qty">Số lượng: ${item.qty||1}</span></div></div>${mats?`<ul class="item-mats">${mats}</ul>`:""}${item.note?`<div class="item-note"><b>Ghi chú</b> ${esc(item.note).replace(/\n/g,"<br>")}</div>`:""}</article>`}).join(""),noteBlock=projectNote?`<section class="note"><h3>Ghi chú dự án</h3><p>${esc(projectNote).replace(/\n/g,"<br>")}</p></section>`:"",moodKeys=Object.keys(allMats),moodboardBlock="";if(moodKeys.length){moodboardBlock=`<section class="moodboard"><h3>Moodboard vật liệu</h3><div class="mood-grid">${moodKeys.map(function(k){var m=allMats[k];return `<div class="mood-card">${m.image?`<img src="${esc(m.image)}">`:'<span class="ph"></span>'}<div><b>${esc(m.type)}</b><br>${esc(m.name)}${m.code?`<br><i>${esc(m.code)}</i>`:""}</div></div>`}).join("")}</div></section>`}var userBlock="";if(uname||uemail){userBlock=`<div class="user-info">${uname?`<b>${esc(uname)}</b><br>`:""}${uemail?`<span>${esc(uemail)}</span>`:""}</div>`}var w=window.open("","_blank");if(!w)return;w.document.write(`<!doctype html><html><head><title>Yêu cầu báo giá dự án COLECTIA</title><style>@page{size:A4;margin:0}*{box-sizing:border-box}body{margin:0;background:#e9e6df;font-family:"Helvetica Neue",Arial,Helvetica,sans-serif;color:#20201e}.sheet{width:210mm;min-height:297mm;background:#fff;margin:auto;padding:15mm 17mm 14mm}.head{position:relative;padding:4mm 0 11mm;border-bottom:2px solid #252421;display:flex;justify-content:space-between;align-items:flex-end}.head-left{flex:1}.logo{display:block;width:126px;height:29px;object-fit:contain;object-position:left;margin-bottom:12px}.eyebrow{font-size:8px;letter-spacing:.24em;color:#817c74;text-transform:uppercase;margin:0 0 5px}.title{font-size:24px;letter-spacing:.1em;font-weight:500;margin:0}.head-right{text-align:right}.date{font-size:9px;color:#706b64;line-height:1.6;margin-bottom:12px}.user-info{font-size:10px;line-height:1.5;color:#20201e;padding-left:14px;border-left:2px solid #20201e;text-align:left;display:inline-block}.user-info b{font-weight:600}.item{padding:8mm 0;border-bottom:1px solid #ded9d0;break-inside:avoid}.product{display:flex;align-items:center;gap:15px}.num{font-size:9px;letter-spacing:.12em;color:#8b867d;min-width:12px}.product>img{width:64px;height:64px;object-fit:cover;background:#f3f1ec}.prod-meta{flex:1}.product h2{font-size:16px;line-height:1.25;font-weight:500;margin:0 0 4px}.product .qty{font-size:10px;color:#706b64;background:#f3f1ec;padding:2px 6px;border-radius:2px}.item-mats{margin:10px 0 0 91px;padding:0;list-style:none;display:flex;flex-wrap:wrap;gap:12px}.item-mats li{display:flex;align-items:center;gap:8px;font-size:9px;line-height:1.35;color:#5b564f;background:#f7f6f3;padding:4px 10px 4px 4px;border-radius:30px}.item-mats img{width:22px;height:22px;border-radius:50%;object-fit:cover}.item-mats b{font-size:7px;letter-spacing:.1em;text-transform:uppercase;color:#817c74}.item-mats i{font-style:normal;color:#8b867d}.item-note{margin:10px 0 0 91px;font-size:10px;line-height:1.5;color:#5b564f}.item-note b{font-size:8px;letter-spacing:.1em;text-transform:uppercase;color:#817c74;margin-right:4px}.moodboard{margin-top:14mm;padding-top:10mm;border-top:1px solid #ded9d0;break-inside:avoid}.moodboard h3,.note h3{font-size:9px;letter-spacing:.16em;text-transform:uppercase;color:#706b64;margin:0 0 12px}.mood-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:12px}.mood-card{font-size:9px;line-height:1.4}.mood-card img,.mood-card .ph{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#f3f1ec;margin-bottom:8px}.mood-card b{font-size:7px;letter-spacing:.1em;text-transform:uppercase;color:#817c74}.mood-card i{font-style:normal;color:#8b867d}.note{margin-top:12mm;padding:8mm 9mm;background:#f7f6f3;break-inside:avoid}.note p{font-size:10px;line-height:1.7;margin:0;color:#45413c}.foot{margin-top:14mm;border-top:2px solid #252421;padding-top:5mm;display:flex;justify-content:space-between;font-size:8px;line-height:1.6;letter-spacing:.02em;color:#777168}@media print{body{background:#fff}.sheet{margin:0}}</style></head><body><main class="sheet"><header class="head"><div class="head-left"><img class="logo" src="https://colectia.vn/wp-content/uploads/2025/10/Logo.svg"><p class="eyebrow">Project selection</p><h1 class="title">YÊU CẦU BÁO GIÁ</h1></div><div class="head-right"><div class="date">Ngày xuất: <b>${new Date().toLocaleDateString("vi-VN")}</b></div>${userBlock}</div></header>${content}${moodboardBlock}${noteBlock}<footer class="foot"><span>COLECTIA VIETNAM<br>colectia.vn</span><span>Danh sách lựa chọn cho dự án của bạn.<br>Vui lòng liên hệ để nhận báo giá chính thức.</span></footer></main></body></html>`);w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>';
	}
