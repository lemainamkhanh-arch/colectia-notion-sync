...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.32.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function render_project_overview() {
		...
		$groups=array(); foreach(WC()->cart->get_cart() as $cart_item){$product=isset($cart_item['data'])?$cart_item['data']:null;if(!$product)continue;$terms=wp_get_post_terms($product->get_id(),'product_cat');$label='Sản phẩm khác';if(!is_wp_error($terms)&&$terms){usort($terms,function($a,$b){return intval($b->parent)-intval($a->parent);});$label=$terms[0]->name;}$groups[$label]=isset($groups[$label])?$groups[$label]+intval($cart_item['quantity']):intval($cart_item['quantity']);}
		if(!$groups)return;ksort($groups,SORT_NATURAL|SORT_FLAG_CASE);echo '<section class="cns-project-overview"><h2>Tổng quan dự án</h2><ul>';foreach($groups as $label=>$qty){echo '<li><span>'.esc_html($label).'</span><strong>'.esc_html($qty).'</strong></li>';}echo '</ul></section>';
	}
