...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.43.3'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	private function catalog_mode_hooks() {
		...
		add_action( 'wp_head', array( $this, 'account_button_styles' ), 160 );
		...
		add_action( 'template_redirect', array( $this, 'render_moodboard_print_view' ), 0 );
		add_action( 'template_redirect', array( $this, 'render_project_quote_print_view' ), 0 );
...
public function render_moodboard_print_view() {
    if(empty($_GET['cns_export_moodboard'])){return;}
    if(!is_user_logged_in()){wp_safe_redirect(wp_login_url(home_url('/my-account/moodboard/')));exit;}
    $materials=$this->get_project_moodboard_materials();
    nocache_headers();status_header(200);header('Content-Type: text/html; charset='.get_bloginfo('charset'));
    $cards='';$listing='';$wood_slot=0;$metal_slot=0;$positions=array(array(4,5,-2),array(51,4,1.5),array(20,36,-1),array(62,35,2),array(37,18,.7),array(3,62,1.2),array(69,64,-1.6),array(40,65,2.2),array(13,20,1),array(55,20,-2),array(25,66,.5),array(72,12,1.3));
    if(!$materials){$cards='<div class="empty-visual">MOODBOARD CHƯA CÓ VẬT LIỆU</div>';$listing='<p class="empty-list">Hãy quay lại Moodboard để thêm vật liệu, sau đó xuất lại.</p>';}
    foreach($materials as $i=>$m){$type=isset($m['type'])?(string)$m['type']:'';$key=function_exists('mb_strtolower')?mb_strtolower($type,'UTF-8'):strtolower($type);$kind='plain';if(strpos($key,'vải')!==false||strpos($key,'fabric')!==false||strpos($key,'da')!==false||strpos($key,'leather')!==false||strpos($key,'nubuck')!==false)$kind='soft';elseif(strpos($key,'đá')!==false||strpos($key,'stone')!==false||strpos($key,'marble')!==false)$kind='stone';elseif(strpos($key,'gỗ')!==false||strpos($key,'wood')!==false)$kind='wood';elseif(strpos($key,'kim loại')!==false||strpos($key,'metal')!==false)$kind='metal';$p=$positions[$i%count($positions)];if($kind==='soft')$p=array(57,24,0);elseif($kind==='stone')$p=(($i%2)?array(35,8,0):array(24,47,0));elseif($kind==='wood'){$wood_positions=array(array(66,68,0),array(5,63,0),array(64,34,0));$p=$wood_positions[$wood_slot%count($wood_positions)];$wood_slot++;}elseif($kind==='metal'){$metal_positions=array(array(13,78,0),array(34,57,0),array(7,36,0));$p=$metal_positions[$metal_slot%count($metal_positions)];$metal_slot++;}else $p=array(7+(($i*22)%65),10+(($i*19)%46),0);$size=$kind==='soft'?42:($kind==='stone'?31:($kind==='wood'?30.6:17));$img=!empty($m['image'])?'<img src="'.esc_url($m['image']).'" alt="'.esc_attr($m['name']).'">':'<span></span>';$cards.='<figure class="sample '.$kind.'" style="left:'.$p[0].'%;top:'.$p[1].'%;width:'.$size.'%;transform:rotate(0deg)">'.$img.'</figure>';$listing.='<article><div class="thumb">'.$img.'</div><small>'.esc_html($type).'</small><b>'.esc_html($m['name']).'</b>'.(!empty($m['code'])?'<em>'.esc_html($m['code']).'</em>':'').'</article>';}
    echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Moodboard COLECTIA</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}html,body{margin:0;background:#fff;font-family:Arial,Helvetica,sans-serif;color:#1b1b19}.sheet{width:210mm;min-height:297mm;margin:auto;padding:12mm;background:#fff}.head{display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:8mm;border-bottom:1px solid #d8d4cd}.eyebrow{font-size:8px;letter-spacing:.18em;color:#777}.title{font-size:25px;font-weight:500;margin:7px 0 0}.date{font-size:9px;color:#777}.visual{position:relative;height:174mm;margin-top:8mm;background:#fff;overflow:hidden}.empty-visual{display:flex;height:100%;align-items:center;justify-content:center;border:1px solid #dedbd5;color:#777;font-size:10px;letter-spacing:.16em}.empty-list{font-size:9px;color:#777}.sample{position:absolute;margin:0;background:#fff;transition:left .26s ease,top .26s ease}.sample img,.sample span{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#f4f2ee}.sample.soft{z-index:4;box-shadow:0 14px 27px rgba(35,31,26,.32),0 5px 10px rgba(35,31,26,.18)}.sample.stone{z-index:3;box-shadow:0 28px 44px rgba(35,31,26,.42),0 9px 15px rgba(35,31,26,.28)}.sample.wood,.sample.metal{z-index:1;box-shadow:none}.list-head{display:flex;justify-content:space-between;padding-bottom:3mm;border-bottom:1px solid #d8d4cd;font-size:7px;letter-spacing:.14em;color:#777}.listing{display:grid;grid-template-columns:repeat(6,1fr);gap:3mm;margin-top:3mm}.listing article{break-inside:avoid}.thumb{width:100%;aspect-ratio:1;background:#f3f1ed;margin-bottom:2mm}.thumb img,.thumb span{display:block;width:100%;height:100%;object-fit:cover}.listing small,.listing b,.listing em{display:block}.listing small{font-size:7.5px;letter-spacing:.1em;text-transform:uppercase;color:#888}.listing b{font-size:9.5px;font-weight:500;margin-top:1mm}.listing em{font-size:7.5px;color:#777;font-style:normal;margin-top:.5mm}.foot{margin-top:8mm;padding-top:4mm;border-top:1px solid #d8d4cd;font-size:7px;color:#777}.tools{position:fixed;right:18px;top:18px;display:flex;align-items:center;gap:9px;z-index:10}.tools button{border:0;background:transparent;color:#6d6962;padding:5px 2px;font:600 8px Arial;letter-spacing:.12em;cursor:pointer}.tools button:hover{color:#1b1b19}.tools .print{border:1px solid #1b1b19;background:#1b1b19;color:#fff;padding:11px 15px;font:600 11px Arial;letter-spacing:.08em}.tools .print:hover{background:#fff;color:#1b1b19}@media print{.tools{display:none}.sheet{margin:0}.sheet{width:auto;min-height:auto}html,body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><div class=tools><button type=button id=cns-recreate>TẠO LẠI</button><button class=print type=button onclick=window.print()>IN / LƯU PDF</button></div><main class="sheet"><header class="head"><div><span class="eyebrow">COLECTIA · MATERIAL PALETTE</span><h1 class="title">MOODBOARD</h1></div><span class="date">'.esc_html(wp_date('d/m/Y')).'</span></header><section class="visual">'.$cards.'</section><div class="list-head"><span>DANH SÁCH VẬT LIỆU</span><span>'.count($materials).' VẬT LIỆU</span></div><section class="listing">'.$listing.'</section><footer class="foot">COLECTIA VIETNAM · colectia.vn</footer></main><script>(function(){var b=document.getElementById("cns-recreate"),visual=document.querySelector(".visual");if(!b||!visual)return;var last=[];function kind(el){return el.classList.contains("soft")?"soft":el.classList.contains("stone")?"stone":el.classList.contains("wood")?"wood":el.classList.contains("metal")?"metal":"plain"}function bounds(k,w,h){if(k==="soft")return [40,58,4,30];if(k==="stone")return [4,66,3,63];if(k==="wood")return [2,70,18,67];if(k==="metal")return [2,80,20,76];return [3,75,5,72]}function overlap(a,c){var iw=Math.max(0,Math.min(a.x+a.w,c.x+c.w)-Math.max(a.x,c.x)),ih=Math.max(0,Math.min(a.y+a.h,c.y+c.h)-Math.max(a.y,c.y)),area=iw*ih;return area>.3*Math.min(a.w*a.h,c.w*c.h)}function create(){var vw=visual.clientWidth,vh=visual.clientHeight,els=Array.prototype.slice.call(visual.querySelectorAll(".sample")),scale=Math.max(.52,1-Math.max(0,els.length-6)*.075);els.forEach(function(el){var base=parseFloat(el.dataset.baseWidth||el.style.width)||17;el.dataset.baseWidth=base;el.style.width=(base*scale).toFixed(2)+"%"});els.sort(function(m,n){var o={soft:0,stone:1,wood:2,metal:3,plain:4};return o[kind(m)]-o[kind(n)]}),placed=[];els.forEach(function(el,index){var r=el.getBoundingClientRect(),w=100*r.width/vw,h=100*r.height/vh,k=kind(el),q=bounds(k,w,h),best=null;for(var i=0;i<90;i++){var maxX=Math.max(q[0],Math.min(q[1],98-w)),maxY=Math.max(q[2],Math.min(q[3],98-h)),c={x:q[0]+Math.random()*(maxX-q[0]),y:q[2]+Math.random()*(maxY-q[2]),w:w,h:h};if(!placed.some(function(p){return overlap(p,c)})){best=c;break}}if(!best){best={x:Math.min(96-w,4+(index*19)%Math.max(5,90-w)),y:Math.min(96-h,5+(index*17)%Math.max(5,90-h)),w:w,h:h}}placed.push(best)});var shift=placed.reduce(function(t,p,i){var old=last[i];return t+(old?Math.abs(p.x-old.x)+Math.abs(p.y-old.y):99)},0);if(shift<45&&last.length===placed.length){return create()}last=placed;placed.forEach(function(p,i){els[i].style.left=p.x.toFixed(2)+"%";els[i].style.top=p.y.toFixed(2)+"%"})}b.addEventListener("click",function(){create();b.textContent="ĐÃ TẠO LẠI";setTimeout(function(){b.textContent="TẠO LẠI"},700)})})();</script></body></html>';exit;
}
...
public function account_button_styles() { if(!function_exists('is_account_page')||!is_account_page())return;echo '<style>.woocommerce-account .woocommerce-MyAccount-content .button,.woocommerce-account .woocommerce-MyAccount-content button[type=submit]{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:0 16px;border:1px solid #1b1b19!important;border-radius:0!important;background:#1b1b19!important;color:#fff!important;font:600 10px/1 inherit!important;letter-spacing:.1em!important;text-transform:uppercase!important;box-shadow:none!important}.woocommerce-account .woocommerce-MyAccount-content .button:hover,.woocommerce-account .woocommerce-MyAccount-content button[type=submit]:hover{background:#fff!important;color:#1b1b19!important}.woocommerce-account .woocommerce-MyAccount-content .button:focus,.woocommerce-account .woocommerce-MyAccount-content button[type=submit]:focus{outline:1px solid #1b1b19;outline-offset:2px}@media(max-width:575px){.woocommerce-account .woocommerce-MyAccount-content .button,.woocommerce-account .woocommerce-MyAccount-content button[type=submit]{width:100%}}</style>'; }
...
public function render_information_account_page() { echo '<div class="cns-account-information"><section class="cns-account-profile"><h2>Thông tin tài khoản</h2>';wc_get_template('myaccount/form-edit-account.php',array('user'=>get_user_by('id',get_current_user_id())));echo '</section><section class="cns-account-addresses"><h2>Địa chỉ</h2>';wc_get_template('myaccount/my-address.php',array('customer_id'=>get_current_user_id()));echo '</section></div><style>.cns-account-information>section+section{margin-top:42px;padding-top:34px;border-top:1px solid #dedbd5}.cns-account-information>section>h2{font-size:22px;font-weight:500;margin:0 0 22px}</style>'; }
	...
	public function project_cart_live_script() { if(!function_exists('is_cart')||!is_cart())return;?>
<script>(function(){function label(){document.querySelectorAll('[name="update_cart"]').forEach(function(b){var v='value'in b?b.value:b.textContent;if(v!=='CẬP NHẬT DỰ ÁN'){if('value'in b)b.value='CẬP NHẬT DỰ ÁN';b.textContent='CẬP NHẬT DỰ ÁN'}})}function clean(){document.querySelectorAll('.cart_totals h2,#cart-totals h2,.cart-collaterals h2,.cart-collaterals h3').forEach(function(h){if((h.textContent||'').trim().toLowerCase().indexOf('tổng cộng giỏ hàng')!==-1)h.remove()})}function sync(){clean();label();document.querySelectorAll('.cns-project-overview li[data-cat]').forEach(function(row){var slug=row.dataset.cat,total=0;document.querySelectorAll('.cart_item.cns-project-cat-'+CSS.escape(slug)+' input.qty').forEach(function(q){total+=parseInt(q.value||0,10)||0});var out=row.querySelector('strong');if(out)out.textContent=total})}function later(){sync();setTimeout(sync,80);setTimeout(sync,350)}document.addEventListener('DOMContentLoaded',later);document.addEventListener('input',function(e){if(e.target.matches('.cart_item input.qty'))later()});document.addEventListener('change',function(e){if(e.target.matches('.cart_item input.qty'))later()});document.addEventListener('click',function(e){if(e.target.closest('.quantity .plus,.quantity .minus,[name="update_cart"]'))later()});if(window.jQuery)jQuery(document.body).on('updated_wc_div updated_cart_totals wc_fragments_refreshed',later)})();</script>
<?php }
	public function project_cart_styles() { if ( ! function_exists('is_cart') || ! is_cart() ) { return; } echo '<style>.woocommerce-cart-form .product-price,.woocommerce-cart-form .product-subtotal,.cart-collaterals .cart-subtotal,.cart-collaterals .order-total{display:none!important}.wd-checkout-steps{display:block!important;list-style:none!important;margin:0 0 22px!important;padding:0!important;text-align:center}.wd-checkout-steps li{display:none!important}.wd-checkout-steps{background:#232323!important;padding:16px 0!important}.wd-checkout-steps:after{content:"PRODUCT LIST";display:block;font-size:20px;font-weight:500;letter-spacing:.2em;color:#fff!important}.cart_totals>h2,.cart_totals .shop_table,.cart_totals table{display:none!important}.cns-project-overview{margin:0 0 25px}.cns-project-overview h2{display:block!important;font-size:13px!important;letter-spacing:.12em!important;text-transform:uppercase!important;margin:0 0 15px!important}.cns-project-overview ul{list-style:none;margin:0;padding:0;border-top:1px solid #d9d5cf}.cns-project-overview li{display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #d9d5cf;font-size:13px}.cns-project-overview strong{font-size:14px;font-weight:600}.woocommerce-cart-form{border-top:1px solid #e4e0da}.woocommerce-cart-form__cart-item td{padding:24px 12px!important;border-bottom:1px solid #e4e0da}.woocommerce-cart-form__cart-item .product-name a{font-size:16px;font-weight:500;color:#1d1d1b}.woocommerce-cart-form__cart-item .variation{margin-top:9px;font-size:12px;color:#777}.woocommerce-cart-form__cart-item .quantity input{width:58px!important;height:34px!important;border:1px solid #d9d5cf!important;font:12px inherit!important}.woocommerce-cart-form__cart-item .product-quantity{display:table-cell!important}.cns-item-note{margin-top:13px}.cns-item-note label{display:block;font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:#777;margin-bottom:5px}.cns-item-note textarea{width:100%;min-height:64px;padding:9px 10px;border:1px solid #ddd8d0;background:#fff;font:12px/1.5 inherit;resize:vertical}.woocommerce-cart-form__cart-item .product-quantity:before{content:"Số lượng";display:block;font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:#777;margin-bottom:5px}.cart-collaterals{background:#f5f3ef;padding:28px}.cart-collaterals .cart_totals>h2,.cart-collaterals .cart_totals>table{display:none!important}.cns-project-note{margin:28px 0 4px;padding:22px;border:1px solid #e3dfd8;background:#faf9f7}.cns-project-note label{display:block;font-size:11px;letter-spacing:.12em;text-transform:uppercase;margin-bottom:9px}.cns-project-note textarea{display:block;width:100%;min-height:100px;padding:12px;border:1px solid #d9d5cf;background:#fff;font:13px/1.6 inherit;resize:vertical}.cns-project-quote{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-height:42px!important;padding:0 16px!important;background:#1b1b19!important;border:1px solid #1b1b19!important;color:#fff!important;font:600 10px/1 inherit!important;letter-spacing:.1em!important;text-transform:uppercase!important}.cns-project-quote:hover{background:#fff!important;color:#1b1b19!important}.cart-collaterals .cns-project-moodboard{margin:0 0 20px;padding:22px;border:1px solid #e3dfd8;background:#faf9f7}.cns-moodboard-head span{font-size:9px;letter-spacing:.16em;text-transform:uppercase;color:#777}.cns-moodboard-head h2{font-size:20px;font-weight:500;margin:5px 0 20px}.cns-moodboard-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.cns-mood-card{background:#fff;border:1px solid #e2ded7}.cns-mood-card>img,.cns-mood-placeholder{display:block;width:100%;aspect-ratio:1;object-fit:cover;background:#e9e6df}.cns-mood-card>div{padding:10px}.cns-mood-card small,.cns-mood-card strong,.cns-mood-card em{display:block}.cns-mood-card small{font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:#777}.cns-mood-card strong{font-size:12px;line-height:1.4;margin-top:3px}.cns-mood-card em{font-size:10px;color:#777;font-style:normal;margin-top:3px}.cns-moodboard-empty{font-size:13px;color:#777}@media(max-width:768px){.wd-checkout-steps{margin-bottom:16px!important;padding:12px 0!important}.wd-checkout-steps:after{font-size:14px!important;letter-spacing:.14em!important}.woocommerce-cart-form__cart-item td{padding:14px 8px!important}.cart-collaterals{padding:16px}.cns-project-overview{margin-bottom:18px}.cns-project-note,.cart-collaterals .cns-project-moodboard{padding:15px;margin-top:16px}.cns-moodboard-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.cns-project-quote,[name=update_cart]{width:100%!important;display:flex!important;justify-content:center!important}.woocommerce-cart-form__cart-item .product-name a{font-size:14px}}@media(max-width:480px){.cns-moodboard-grid{grid-template-columns:1fr}.cns-mood-card{display:grid;grid-template-columns:82px 1fr}.cns-mood-card>img,.cns-mood-placeholder{height:82px;aspect-ratio:auto}.cns-project-note textarea{min-height:76px}.cart-collaterals{padding:12px}}</style>'; }
	...
	public function render_project_quote_print_view() {
		if(empty($_GET['cns_export_project_quote']))return;
		if(!function_exists('WC')||!WC()->cart||WC()->cart->is_empty()){wp_safe_redirect(function_exists('wc_get_cart_url')?wc_get_cart_url():home_url('/cart/'));exit;}
		nocache_headers();status_header(200);header('Content-Type: text/html; charset='.get_bloginfo('charset'));
		$rows='';$number=0;foreach(WC()->cart->get_cart() as $item){$product=isset($item['data'])?$item['data']:null;if(!$product)continue;$number++;$pid=$product->get_id();$image=get_the_post_thumbnail_url($pid,'medium');if(!$image&&function_exists('wc_placeholder_img_src'))$image=wc_placeholder_img_src('medium');$materials='';foreach((array)(isset($item['cns_selected_materials'])?$item['cns_selected_materials']:array()) as $m){if(empty($m['name']))continue;$materials.='<li>'.(!empty($m['image'])?'<img src="'.esc_url($m['image']).'" alt="">':'').'<span><small>'.esc_html(isset($m['type'])?$m['type']:'Vật liệu').'</small><b>'.esc_html($m['name']).'</b>'.(!empty($m['code'])?'<em>'.esc_html($m['code']).'</em>':'').'</span></li>';}$note=!empty($item['cns_product_note'])?'<p class="item-note"><b>GHI CHÚ</b> '.nl2br(esc_html($item['cns_product_note'])).'</p>':'';$rows.='<article class="item"><span class="num">'.str_pad((string)$number,2,'0',STR_PAD_LEFT).'</span>'.($image?'<img class="product-image" src="'.esc_url($image).'" alt="'.esc_attr($product->get_name()).'">':'').'<div class="product-info"><h2>'.esc_html($product->get_name()).'</h2><p class="qty">Số lượng: '.intval(isset($item['quantity'])?$item['quantity']:1).'</p>'.($materials?'<ul>'.$materials.'</ul>':'').$note.'</div></article>';}
		$project_note=WC()->session?WC()->session->get('cns_project_note',''):'';$note_block=$project_note?'<section class="project-note"><span>GHI CHÚ DỰ ÁN</span><p>'.nl2br(esc_html($project_note)).'</p></section>':'';
		$customer=get_current_user_id()?get_userdata(get_current_user_id()):null;$customer_name=$customer?trim($customer->display_name):'';$customer_email=$customer?$customer->user_email:'';$customer_block=($customer_name||$customer_email)?'<section class="customer"><span>THÔNG TIN KHÁCH HÀNG</span>'.($customer_name?'<b>'.esc_html($customer_name).'</b>':'').($customer_email?'<em>'.esc_html($customer_email).'</em>':'').'</section>':'';$rows=$customer_block.$rows;
		echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Yêu cầu báo giá COLECTIA</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}html,body{margin:0;background:#eeeae4;color:#1b1b19;font-family:Arial,Helvetica,sans-serif}.sheet{width:210mm;min-height:297mm;margin:auto;padding:15mm 18mm;background:#fff}.print{position:fixed;right:18px;top:18px;border:1px solid #1b1b19;background:#1b1b19;color:#fff;padding:11px 15px;font:600 11px Arial;letter-spacing:.08em;cursor:pointer}.head{display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:9mm;border-bottom:1px solid #d8d4cd}.logo{width:126px;height:29px;object-fit:contain;object-position:left}.eyebrow{font-size:8px;letter-spacing:.2em;color:#777;margin:10px 0 5px}.head h1{font-size:23px;letter-spacing:.08em;font-weight:500;margin:0}.date{font-size:9px;color:#777}.item{display:grid;grid-template-columns:18px 70px 1fr;gap:13px;padding:7mm 0;border-bottom:1px solid #e6e2dc;break-inside:avoid}.num{font-size:8px;color:#888}.product-image{width:70px;height:70px;object-fit:cover;background:#f4f2ee}.product-info h2{font-size:15px;font-weight:500;margin:0 0 4px}.qty{font-size:9px;color:#666;margin:0 0 8px}.product-info ul{display:flex;flex-wrap:wrap;gap:7px;list-style:none;margin:0;padding:0}.product-info li{display:flex;align-items:center;gap:6px;border:1px solid #ece8e2;padding:4px 7px 4px 4px}.product-info li img{width:26px;height:26px;object-fit:cover}.product-info li small,.product-info li b,.product-info li em{display:block}.product-info li small{font-size:6px;letter-spacing:.08em;color:#888}.product-info li b{font-size:8px;font-weight:500}.product-info li em{font-size:7px;color:#777;font-style:normal}.item-note{font-size:8px;color:#666;margin:8px 0 0}.project-note{margin-top:8mm;padding:7mm;background:#f7f5f1}.project-note span{font-size:7px;letter-spacing:.16em;color:#777}.project-note p{font-size:9px;line-height:1.6;margin:5px 0 0}.foot{display:flex;justify-content:space-between;margin-top:10mm;padding-top:5mm;border-top:1px solid #d8d4cd;font-size:7px;color:#777}@media print{html,body{background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.sheet{width:auto;min-height:auto;margin:0}.print{display:none}}@media(max-width:760px){.sheet{width:100%;min-height:0;padding:24px 18px}.head h1{font-size:18px}.logo{width:105px}.item{grid-template-columns:14px 58px 1fr;gap:9px}.product-image{width:58px;height:58px}.print{right:10px;top:10px}}.head{position:relative;justify-content:center!important;text-align:center}.head>div{margin:0 auto}.head .logo{display:block;margin:0 auto}.head .date{position:absolute;right:0;bottom:9mm}.customer{display:flex;flex-direction:column;align-items:center;gap:3px;margin:0 0 8mm;padding:4mm 0;border-top:1px solid #d8d3ca;border-bottom:1px solid #d8d3ca;text-align:center}.customer span{font-size:8px;letter-spacing:.14em;color:#777}.customer b{font-size:10px;font-weight:600}.customer em{font-size:9px;font-style:normal;color:#666}@media(max-width:760px){.head .date{position:static;display:block;margin-top:10px}}</style></head><body><button class="print" type="button" onclick="window.print()">IN / LƯU PDF</button><main class="sheet"><header class="head"><div><img class="logo" src="https://colectia.vn/wp-content/uploads/2025/10/Logo.svg" alt="COLECTIA"><p class="eyebrow">PROJECT SELECTION</p><h1>YÊU CẦU BÁO GIÁ</h1></div><span class="date">'.esc_html(wp_date('d/m/Y')).'</span></header>'.$rows.$note_block.'<footer class="foot"><span>COLECTIA VIETNAM · colectia.vn</span><span>Vui lòng liên hệ để nhận báo giá chính thức.</span></footer></main></body></html>';exit;
	}

	public function project_quote_button() { if(!function_exists('WC')||!WC()->cart||WC()->cart->is_empty())return;$url=add_query_arg('cns_export_project_quote','1',wc_get_cart_url());echo "<a class=\"checkout-button cns-project-quote\" target=\"_blank\" rel=\"noopener\" href=\"".esc_url($url)."\">XUẤT YÊU CẦU BÁO GIÁ</a>"; }
