...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.35.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function project_quote_button() {
		...
		echo '<script>(function(){var b=document.querySelector(".cns-project-quote");if(!b)return;b.addEventListener("click",function(e){e.preventDefault();var note=document.querySelector("#cns_project_note"),items=JSON.parse(b.dataset.project||"[]"),projectNote=note?note.value:b.dataset.note||"",esc=function(v){var d=document.createElement("div");d.textContent=v||"";return d.innerHTML},content=items.map(function(item,i){var mats=(item.materials||[]).map(function(m){return `<li>${m.image?`<img src="${esc(m.image)}">`:""}<span><b>${esc(m.type||"Vật liệu")}</b><br>${esc(m.name)}${m.code?` · ${esc(m.code)}`:""}</span></li>`}).join("");return `<article class="item"><div class="product"><span class="num">${String(i+1).padStart(2,"0")}</span><img src="${esc(item.image)}"><div><h2>${esc(item.name)}</h2><p class="qty">Số lượng: ${item.qty||1}</p></div></div>${mats?`<div class="materials"><h3>Vật liệu đã chọn</h3><ul>${mats}</ul></div>`:""}${item.note?`<div class="product-note"><b>Ghi chú</b><br>${esc(item.note).replace(/\n/g,"<br>")}</div>`:""}</article>`}).join(""),noteBlock=projectNote?`<section class="note"><h3>Ghi chú dự án</h3><p>${esc(projectNote).replace(/\n/g,"<br>")}</p></section>`:"",w=window.open("","_blank");if(!w)return;w.document.write(`<!doctype html><html><head><title>Yêu cầu báo giá dự án COLECTIA</title><style>@page{size:A4;margin:0}*{box-sizing:border-box}body{margin:0;background:#e9e6df;font-family:"Helvetica Neue",Arial,Helvetica,sans-serif;color:#20201e}.sheet{width:210mm;min-height:297mm;background:#fff;margin:auto;padding:15mm 17mm 14mm}.head{position:relative;padding:4mm 0 11mm;border-bottom:2px solid #252421;text-align:center}.logo{position:absolute;left:0;top:2mm;width:126px;height:29px;object-fit:contain;object-position:left}.eyebrow{font-size:8px;letter-spacing:.24em;color:#817c74;text-transform:uppercase;margin:11mm 0 7px}.title{font-size:27px;letter-spacing:.1em;font-weight:500;margin:0}.date{position:absolute;right:0;top:3mm;font-size:9px;color:#706b64;text-align:right;line-height:1.65}.item{padding:7mm 0;border-bottom:1px solid #ded9d0;break-inside:avoid}.product{display:grid;grid-template-columns:12mm 60px 1fr;align-items:center;gap:12px}.num{font-size:9px;letter-spacing:.12em;color:#8b867d}.product>img{width:60px;height:60px;object-fit:cover;background:#f3f1ec}.product h2{font-size:16px;line-height:1.25;font-weight:500;margin:0}.product .qty{margin:4px 0 0;font-size:9px;letter-spacing:.05em;text-transform:uppercase;color:#7e7870}.product-note{margin:7px 0 0 84px;padding-left:12px;border-left:1px solid #d8d3ca;font-size:9px;line-height:1.55;color:#5b564f}.product-note b{font-size:8px;letter-spacing:.12em;text-transform:uppercase;color:#817c74}.materials{margin:7px 0 0 84px;padding:8px 10px;border-left:2px solid #252421;background:#f7f6f3}.materials h3,.note h3{font-size:8px;letter-spacing:.16em;text-transform:uppercase;color:#706b64;margin:0 0 7px}.materials ul{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px 14px}.materials li{display:flex;align-items:center;gap:7px;font-size:9px;line-height:1.3}.materials li img{width:28px;height:28px;object-fit:cover}.materials b{font-size:7px;letter-spacing:.11em;text-transform:uppercase;color:#706b64}.note{margin-top:10mm;padding:8mm 9mm;border:1px solid #dcd7ce;background:#f7f6f3}.note p{font-size:10px;line-height:1.7;margin:0;color:#45413c}.foot{margin-top:12mm;border-top:2px solid #252421;padding-top:5mm;display:flex;justify-content:space-between;font-size:8px;line-height:1.6;letter-spacing:.02em;color:#777168}@media print{body{background:#fff}.sheet{margin:0}}</style></head><body><main class="sheet"><header class="head"><img class="logo" src="https://colectia.vn/wp-content/uploads/2025/10/Logo.svg"><p class="eyebrow">Project selection</p><h1 class="title">YÊU CẦU BÁO GIÁ</h1><p class="date">Ngày xuất<br><b>${new Date().toLocaleDateString("vi-VN")}</b></p></header>${content}${noteBlock}<footer class="foot"><span>COLECTIA VIETNAM<br>colectia.vn</span><span>Danh sách lựa chọn cho dự án của bạn.<br>Vui lòng liên hệ để nhận báo giá chính thức.</span></footer></main></body></html>`);w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>';
	}
