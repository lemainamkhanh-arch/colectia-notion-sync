...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.30.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	public function store_selected_project_materials( $cart_item_data, $product_id, $variation_id ) {
		$raw=isset($_POST['cns_selected_materials'])?json_decode(wp_unslash($_POST['cns_selected_materials']),true):array();$chosen=array();if(is_array($raw)){foreach($raw as $m){if(empty($m['name']))continue;$chosen[]=array('type'=>sanitize_text_field(isset($m['type'])?$m['type']:''),'name'=>sanitize_text_field($m['name']),'code'=>sanitize_text_field(isset($m['code'])?$m['code']:''),'image'=>esc_url_raw(isset($m['image'])?$m['image']:''));}}if($chosen){$cart_item_data['cns_selected_materials']=$chosen;$cart_item_data['cns_project_key']=md5(wp_json_encode($chosen));}if(!empty($_POST['cns_product_note'])){$cart_item_data['cns_product_note']=sanitize_textarea_field(wp_unslash($_POST['cns_product_note']));}return $cart_item_data;
	}

	public function show_selected_project_materials( $data, $cart_item ) { if(!empty($cart_item['cns_selected_materials'])&&is_array($cart_item['cns_selected_materials'])){$names=array();foreach($cart_item['cns_selected_materials'] as $m){$names[]=trim((!empty($m['type'])?$m['type'].': ':'').$m['name']);}$data[]=array('name'=>'Vật liệu đã chọn','value'=>implode(', ',$names),'display'=>implode(', ',$names));}if(!empty($cart_item['cns_product_note'])){$data[]=array('name'=>'Ghi chú','value'=>esc_html($cart_item['cns_product_note']),'display'=>esc_html($cart_item['cns_product_note']));}return $data; }
	...
	public function project_cart_styles() { if ( ! function_exists('is_cart') || ! is_cart() ) { return; } echo '<style>.woocommerce-cart-form .product-quantity,.woocommerce-cart-form .product-price,.cart-collaterals .cart-subtotal,.cart-collaterals .order-total{display:none!important}.woocommerce-cart-form{border-top:1px solid #e4e0da}.woocommerce-cart-form__cart-item td{padding:24px 12px!important;border-bottom:1px solid #e4e0da}.woocommerce-cart-form__cart-item .product-name a{font-size:16px;font-weight:500;color:#1d1d1b}.woocommerce-cart-form__cart-item .variation{margin-top:9px;font-size:12px;color:#777}.woocommerce-cart-form__cart-item .quantity input{width:58px!important;height:34px!important;border:1px solid #d9d5cf!important;font:12px inherit!important}.woocommerce-cart-form__cart-item .product-quantity{display:table-cell!important}.woocommerce-cart-form__cart-item .product-quantity:before{content:"Số lượng";display:block;font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:#777;margin-bottom:5px}.cart-collaterals{background:#f5f3ef;padding:28px}.cart-collaterals .cart_totals>h2,.cart-collaterals .cart_totals>table{display:none!important}.cns-project-note{margin:28px 0 4px;padding:22px;border:1px solid #e3dfd8;background:#faf9f7}.cns-project-note label{display:block;font-size:11px;letter-spacing:.12em;text-transform:uppercase;margin-bottom:9px}.cns-project-note textarea{display:block;width:100%;min-height:100px;padding:12px;border:1px solid #d9d5cf;background:#fff;font:13px/1.6 inherit;resize:vertical}.cns-project-quote{background:#1b1b19!important;border:1px solid #1b1b19!important;color:#fff!important;padding:15px 20px!important;font-size:12px!important;letter-spacing:.08em;text-transform:uppercase}.cns-project-quote:hover{background:transparent!important;color:#1b1b19!important}</style>'; }
	...
	public function save_project_note() { if ( function_exists('WC') && WC()->session && isset($_POST['cns_project_note']) ) { WC()->session->set('cns_project_note',sanitize_textarea_field(wp_unslash($_POST['cns_project_note']))); } }
	...
	public function project_quote_button() {
		...
		if(!$items)return;$note=WC()->session?WC()->session->get('cns_project_note',''):'';echo '<a href="#" class="checkout-button button alt cns-project-quote" data-project="'.esc_attr(wp_json_encode($items)).'" data-note="'.esc_attr($note).'">Xuất báo giá dự án</a>';
		echo '<script>(function(){var b=document.querySelector(".cns-project-quote");if(!b)return;b.addEventListener("click",function(e){e.preventDefault();var note=document.querySelector("#cns_project_note"),items=JSON.parse(b.dataset.project||"[]"),projectNote=note?note.value:b.dataset.note||"",esc=function(v){var d=document.createElement("div");d.textContent=v||"";return d.innerHTML},content=items.map(function(item,i){var mats=(item.materials||[]).map(function(m){return `<li>${m.image?`<img src="${esc(m.image)}">`:""}<span><b>${esc(m.type||"Vật liệu")}</b><br>${esc(m.name)}${m.code?` · ${esc(m.code)}`:""}</span></li>`}).join("");return `<article class="item"><div class="product"><span class="num">${String(i+1).padStart(2,"0")}</span><img src="${esc(item.image)}"><div><h2>${esc(item.name)}</h2><p class="qty">Số lượng: ${item.qty||1}</p></div></div>${mats?`<div class="materials"><h3>Vật liệu đã chọn</h3><ul>${mats}</ul></div>`:""}${item.note?`<div class="product-note"><b>Ghi chú</b><br>${esc(item.note).replace(/\n/g,"<br>")}</div>`:""}</article>`}).join(""),noteBlock=projectNote?`<section class="note"><h3>Ghi chú dự án</h3><p>${esc(projectNote).replace(/\n/g,"<br>")}</p></section>`:"",w=window.open("","_blank");if(!w)return;w.document.write(`<!doctype html><html><head><title>Báo giá dự án COLECTIA</title><style>@page{size:A4;margin:0}*{box-sizing:border-box}body{margin:0;background:#f1efe9;font-family:"Helvetica Neue",Arial,Helvetica,sans-serif;color:#1d1d1b}.sheet{width:210mm;min-height:297mm;background:#fff;margin:auto;padding:18mm 20mm 17mm}.head{text-align:center;border-bottom:1px solid #d8d4cc;padding:7mm 0 14mm;position:relative}.logo{position:absolute;left:0;top:0;width:138px;height:30px;object-fit:contain;object-position:left}.eyebrow{font-size:9px;letter-spacing:.2em;color:#79756e;text-transform:uppercase;margin:14px 0 8px}.title{font-size:31px;letter-spacing:.12em;font-weight:500;margin:0}.date{position:absolute;right:0;top:0;font-size:10px;color:#79756e;text-align:right;line-height:1.65}.item{padding:12mm 0;border-bottom:1px solid #e5e1da}.product{display:flex;align-items:center;gap:14px}.num{font-size:10px;letter-spacing:.1em;color:#8b867d}.product>img{width:78px;height:78px;object-fit:cover}.product h2{font-size:18px;line-height:1.35;font-weight:500;margin:0}.product .qty{margin:5px 0 0;font-size:10px;color:#79756e}.product-note{margin:8px 0 0 102px;font-size:10px;line-height:1.6;color:#56514a}.product-note b{font-size:8px;letter-spacing:.1em;text-transform:uppercase;color:#79756e}.materials{margin:10px 0 0 102px;padding:9px 11px;background:#faf9f6}.materials h3,.note h3{font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:#79756e;margin:0 0 7px}.materials ul{list-style:none;margin:0;padding:0;display:flex;flex-wrap:wrap;gap:10px 18px}.materials li{display:flex;align-items:center;gap:7px;font-size:10px;line-height:1.35}.materials li img{width:30px;height:30px;object-fit:cover}.materials b{font-size:8px;letter-spacing:.1em;text-transform:uppercase;color:#79756e}.note{margin-top:15mm;padding:11mm;border:1px solid #e2ded6;background:#faf9f6}.note p{font-size:11px;line-height:1.7;margin:0;color:#45413c}.foot{margin-top:18mm;border-top:1px solid #d8d4cc;padding-top:7mm;display:flex;justify-content:space-between;font-size:9px;line-height:1.6;color:#79756e}@media print{body{background:#fff}.sheet{margin:0}}</style></head><body><main class="sheet"><header class="head"><img class="logo" src="https://colectia.vn/wp-content/uploads/2025/10/Logo.svg"><p class="eyebrow">Project selection</p><h1 class="title">BÁO GIÁ</h1><p class="date">Ngày xuất<br><b>${new Date().toLocaleDateString("vi-VN")}</b></p></header>${content}${noteBlock}<footer class="foot"><span>COLECTIA VIETNAM<br>colectia.vn</span><span>Danh sách sản phẩm dành cho dự án của bạn.<br>Vui lòng liên hệ để nhận báo giá chính thức.</span></footer></main></body></html>`);w.document.close();w.onload=function(){w.focus();w.print()}})})();</script>';
	}
