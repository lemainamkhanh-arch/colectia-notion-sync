...
class Colectia_Notion_Sync {
	...
	const PLUGIN_VERSION = '1.29.0'; // Tăng số này để buộc đồng bộ lại toàn bộ, kể cả trang không đổi
...
';
	...
	private function catalog_mode_hooks() {
		...
		add_filter( 'woocommerce_cart_item_quantity', '__return_empty_string', 99 );
	...
	public function store_selected_project_materials( $cart_item_data, $product_id, $variation_id ) {
		if ( empty( $_POST['cns_selected_materials'] ) ) { return $cart_item_data; }
		$raw = json_decode( wp_unslash( $_POST['cns_selected_materials'] ), true ); if ( ! is_array( $raw ) ) { return $cart_item_data; }
		$chosen = array(); foreach ( $raw as $m ) { if ( empty($m['name']) ) { continue; } $chosen[] = array( 'type'=>sanitize_text_field(isset($m['type'])?$m['type']:''), 'name'=>sanitize_text_field($m['name']), 'code'=>sanitize_text_field(isset($m['code'])?$m['code']:''), 'image'=>esc_url_raw(isset($m['image'])?$m['image']:'') ); }
		if ( $chosen ) { $cart_item_data['cns_selected_materials'] = $chosen; $cart_item_data['cns_project_key'] = md5( wp_json_encode( $chosen ) ); }
		return $cart_item_data;
	}

	public function show_selected_project_materials( $data, $cart_item ) {
		if ( empty( $cart_item['cns_selected_materials'] ) || ! is_array( $cart_item['cns_selected_materials'] ) ) { return $data; }
		$names=array(); foreach($cart_item['cns_selected_materials'] as $m){$names[]=trim((!empty($m['type'])?$m['type'].': ':'').$m['name']);}
		$data[] = array( 'name'=>'Vật liệu đã chọn', 'value'=>implode(', ',$names), 'display'=>implode(', ',$names) ); return $data;
	}
	...
	public function project_cart_styles() { if ( ! function_exists('is_cart') || ! is_cart() ) { return; } echo '<style>.woocommerce-cart-form .product-quantity,.woocommerce-cart-form .product-price,.cart-collaterals .cart-subtotal,.cart-collaterals .order-total{display:none!important}.woocommerce-cart-form{border-top:1px solid #e4e0da}.woocommerce-cart-form__cart-item td{padding:24px 12px!important;border-bottom:1px solid #e4e0da}.woocommerce-cart-form__cart-item .product-name a{font-size:16px;font-weight:500;color:#1d1d1b}.woocommerce-cart-form__cart-item .variation{margin-top:9px;font-size:12px;color:#777}.cart-collaterals{background:#f5f3ef;padding:28px}.cart-collaterals .cart_totals>h2,.cart-collaterals .cart_totals>table{display:none!important}.cns-project-note{margin:28px 0 4px;padding:22px;border:1px solid #e3dfd8;background:#faf9f7}.cns-project-note label{display:block;font-size:11px;letter-spacing:.12em;text-transform:uppercase;margin-bottom:9px}.cns-project-note textarea{display:block;width:100%;min-height:100px;padding:12px;border:1px solid #d9d5cf;background:#fff;font:13px/1.6 inherit;resize:vertical}.cns-project-quote{background:#1b1b19!important;border:1px solid #1b1b19!important;color:#fff!important;padding:15px 20px!important;font-size:12px!important;letter-spacing:.08em;text-transform:uppercase}.cns-project-quote:hover{background:transparent!important;color:#1b1b19!important}</style>'; }
	...
	public function project_quote_button() {
		...
		foreach ( WC()->cart->get_cart() as $cart_item ) { $product=isset($cart_item['data'])?$cart_item['data']:null;if(!$product)continue;$pid=$product->get_id();$image=get_the_post_thumbnail_url($pid,'medium');if(!$image)$image=wc_placeholder_img_src('medium');$items[]=array('name'=>$product->get_name(),'image'=>$image,'materials'=>(!empty($cart_item['cns_selected_materials'])&&is_array($cart_item['cns_selected_materials']))?$cart_item['cns_selected_materials']:array()); }
